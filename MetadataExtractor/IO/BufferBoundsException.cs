...
{
    /// <summary>
    /// Thrown when the index provided to a <see cref="ReaderInfo"/> is invalid.
    /// </summary>
    ...
    public class BufferBoundsException : IOException
    {
        ...
        private static string GetMessage(long index, long bytesRequested, long bufferLength)
        {
            ...
            if (index + bytesRequested - 1L > long.MaxValue)
                return $"Number of requested bytes summed with starting index exceed maximum range of signed 64 bit integers (requested index: {index}, requested count: {bytesRequested})";
