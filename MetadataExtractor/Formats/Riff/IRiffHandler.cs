...
namespace MetadataExtractor.Formats.Riff
{
    ...
    public interface IRiffHandler
    {
        ...
        /// to <see cref="ProcessChunk(string, ReaderInfo)"/>, or <c>false</c> to avoid
        /// the copy and skip to the next chunk in the file, if any.
        /// </remarks>
        /// <param name="fourCc">the four character code of this chunk</param>
        /// <returns><c>true</c> if <see cref="ProcessChunk(string, ReaderInfo)"/> should be called, otherwise <c>false</c>.</returns>
        ...
        void ProcessChunk([NotNull] string fourCc, [NotNull] ReaderInfo payload);
    }
}
