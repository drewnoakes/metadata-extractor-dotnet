...
namespace MetadataExtractor.Formats.Icc
{
    ...
    public sealed class IccReader : IJpegSegmentMetadataReader
    {
        ...
        public DirectoryList ReadJpegSegments(IEnumerable<JpegSegment> segments)
        {
            ...
            var iccSegments = segments.Where(segment => segment.Reader.Length > JpegSegmentPreambleLength && JpegSegmentId.Equals(segment.Preamble, StringComparison.OrdinalIgnoreCase)).ToList();

            if (iccSegments.Count == 0)
                return new Directory[0];

            ReaderInfo iccInfo = null;
            if (iccSegments.Count == 1)
            {
                iccInfo = iccSegments[0].Reader.Clone(JpegSegmentPreambleLength, iccSegments[0].Reader.Length - JpegSegmentPreambleLength);
            }
            else
            {
        ...
        private static void Set4ByteString([NotNull] Directory directory, int tagType, [NotNull] ReaderInfo reader)
        {
        ...
        private static void SetInt32([NotNull] Directory directory, int tagType, [NotNull] ReaderInfo reader)
        {
        ...
        private static void SetInt64([NotNull] Directory directory, int tagType, [NotNull] ReaderInfo reader)
        {
        ...
        private static void SetDate([NotNull] IccDirectory directory, int tagType, [NotNull] ReaderInfo reader)
        {
